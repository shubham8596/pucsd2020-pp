...
export class UpdateuserdetailsComponent implements OnInit {
  ...
  users  = [];
    ...
    openDialog(user,id,firstname,lastname,password) {

      id : user.data['u_id'],
      firstname="daud",
      lastname= user.data['u_lastname'],
      password= user.data['u_password'],
     // user.data['first_name']="madhav"
     // console.log("User : "+ user + "firstname : "+user.data['first_name']+"id : "+id+" email : "+email);
      this.dataService.updateById(user,user.data['u_id'],user.data['u_firstname'],user.data['u_lastname'],user.data['u_password']).subscribe(data => {
