...
func (group *Group) GetHTTPHandler() []*handler.HTTPHandler {
	return []*handler.HTTPHandler{
		&handler.HTTPHandler{Authenticated: true, Method: http.MethodGet, Path: "group/{g_id}", Func: group.GetByID},
		&handler.HTTPHandler{Authenticated: true, Method: http.MethodPost, Path: "group", Func: group.Create},
		&handler.HTTPHandler{Authenticated: true, Method: http.MethodPut, Path: "group/{g_id}", Func: group.Update},
		&handler.HTTPHandler{Authenticated: true, Method: http.MethodDelete, Path: "group/{g_id}", Func: group.Delete},
...
func (group *Group) GetByID(w http.ResponseWriter, r *http.Request) {
	var grp interface{}
	id, err := strconv.ParseInt(chi.URLParam(r, "g_id"), 10, 64)
	for {
		if nil != err {
			break
		}
...
func (group *Group) Update(w http.ResponseWriter, r *http.Request) {
	var iGrp interface{}
	id, _ := strconv.ParseInt(chi.URLParam(r, "g_id"), 10, 64)
	grp := model.Group{}
	err := json.NewDecoder(r.Body).Decode(&grp)
	for {
		if nil != err {
