...
func GetByEmail(conn *sql.DB, object model.IModel, email string) (model.IModel, error) {
	rValue := reflect.ValueOf(object)
	rType := reflect.TypeOf(object)
	...
	queryBuffer.WriteString(" FROM ")
	queryBuffer.WriteString(object.Table())
	queryBuffer.WriteString(" WHERE email = ?")

	query := queryBuffer.String()
	//	log.Printf("GetById sql: %s\n", query)
	row, err := conn.Query(query, email)
	...
	if row.Next() {
		...
		return nil, errors.New(fmt.Sprintf("Entry not found for email: %s", email))
	}
